...
import com.netdisk.entity.dto.UserSpaceDto;
import com.netdisk.entity.enums.*;
...
public class FileInfoServiceImpl implements FileInfoService {
    ...
    public void transferFile(String fileId, SessionWebUserDto webUserDto) {
        ...
        try {
            ...
            //视频文件切割
            String ffmpegPath = appConfig.getProjectFfmpegPath();
            ...
            if (FileTypeEnums.VIDEO == fileTypeEnum) {
                ...
                ScaleFilter.createCover4Video(new File(targetFilePath), Constants.LENGTH_150, new File(coverPath), ffmpegPath);
            } else if (FileTypeEnums.IMAGE == fileTypeEnum) {
                //生成缩略图
                cover = month + "/" + realFileName.replace(".", "_.");
                String coverPath = targetFolderName + "/" + cover;
                Boolean created = ScaleFilter.createThumbnailWidthFFmpeg(new File(targetFilePath), Constants.LENGTH_150, new File(coverPath), false, ffmpegPath);
                if (!created) {
    ...
    private void cutFile4Video(String fileId, String videoFilePath) {
        ...
        String ffmpegPath = appConfig.getProjectFfmpegPath();
        String ffprobe = StringTools.isEmpty(ffmpegPath) ? "ffprobe" : ffmpegPath + "/ffprobe";
    ...
    public FileInfo rename(String fileId, String userId, String fileName) {
        ...
        Integer count = this.fileInfoMapper.selectCount(fileInfoQuery);
        if (count > 1) {
            throw new BusinessException("文件" + fileName + "已经存在");
        }
        fileInfo.setFileName(fileName);
        fileInfo.setLastUpdateTime(curDate);
        return fileInfo;
    ...
    public FileInfo newFolder(String filePid, String userId, String folderName) {
        ...
        Integer count = this.fileInfoMapper.selectCount(fileInfoQuery);
        if (count > 1) {
            throw new BusinessException("文件" + folderName + "已经存在");
        }
        fileInfo.setFileName(folderName);
        fileInfo.setLastUpdateTime(curDate);
        return fileInfo;
    ...
    public void recoverFileBatch(String userId, String fileIds) {
        ...
        //将选中的文件更新为正常,且父级目录到跟目录的文件状态也更新为正常
        List<String> delFileIdList = Arrays.asList(fileIdArray);
        FileInfo fileInfo = new FileInfo();
        fileInfo.setDelFlag(FileDelFlagEnums.USING.getFlag());
